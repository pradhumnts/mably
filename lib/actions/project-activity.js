...
import {
  isDemoProjectId,
  getDemoProjectActivityPageData,
...
export async function getProjectActivityPageData(projectId) {
  ...
  if (isDemoProjectId(pid)) {
    ...
    return getDemoProjectActivityPageData(freelancer);
  }
  ...
  const { data: events, error: eventsErr } = await supabase
    ...
    .select(
      "id, event_type, actor_id, actor_display_name, actor_avatar_url, payload, created_at"
    )
    .eq("project_id", pid)
  ...
  const overlaidEvents = await overlayLiveUploaderIdentities(
    (events ?? []).map((row) => ({
      ...row,
  ...
  const activities = overlaidEvents.map((row) =>
    mapActivityEventRowToTimelineItem(
      {
        ...row,
