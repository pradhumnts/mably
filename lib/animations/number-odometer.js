...
export function createNumberOdometer() {
  ...
  /**
   * Lock final glyphs in place without replacing the whole node with plain text
   * (avoids end-of-animation width snap). Removes rollers and GSAP inline sizes.
   */
  function settleElement(el, originalText) {
    gsap.killTweensOf(el);

    const segments = parseSegments(originalText);
    const digitChars = segments.filter((s) => s.type === "digit").map((s) => s.char);
    const staticChars = segments.filter((s) => s.type === "static").map((s) => s.char);

    const masks = [...el.querySelectorAll('[data-odometer-part="mask"]')];
    while (masks.length > digitChars.length) {
      masks.shift()?.remove();
    }
    ...
    masks.forEach((mask, index) => {
      ...
      mask.textContent = digitChars[index] ?? "";
      gsap.set(mask, { clearProps: "width,opacity,padding,margin" });
      mask.style.height = `${step}em`;
      mask.style.lineHeight = String(step);
      mask.style.overflow = "hidden";
    });

    const statics = [...el.querySelectorAll('[data-odometer-part="static"]')];
    statics.forEach((span, index) => {
      const ch = staticChars[index];
      if (ch === undefined) return;
      span.textContent = ch === " " ? "\u00a0" : ch;
      gsap.set(span, { clearProps: "width,opacity,padding,margin" });
      span.style.height = `${step}em`;
      span.style.lineHeight = String(step);
    });

    if (masks.length !== digitChars.length || statics.length !== staticChars.length) {
      buildSettledDOM(el, originalText, step);
      return;
    }
  ...
  /** Rebuild a clean settled DOM when in-place settle cannot match segment counts. */
  function buildSettledDOM(el, originalText, step) {
    gsap.killTweensOf(el);
    el.innerHTML = "";
    el.style.overflow = "";
    el.style.height = "";
    ...
    parseSegments(originalText).forEach((seg) => {
      if (seg.type === "static") {
        const span = document.createElement("span");
        span.setAttribute("data-odometer-part", "static");
        span.style.height = `${step}em`;
