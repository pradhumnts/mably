...
import { overlayLiveUploaderIdentities } from "@/lib/auth/resolve-actor-identity";
  ...
  const counts = await Promise.all(
    convs.map(async (conv) => {
      ...
      const { data: latestMsg } = await supabase
        .from("project_messages")
        .select("id, created_at, author_display_name, author_avatar_url")
        .eq("conversation_id", conv.id)
        .neq("author_id", userId)
        .gt("created_at", since)
      ...
      return {
        ...
        actorName:
          (typeof latestMsg.author_display_name === "string" &&
            latestMsg.author_display_name.trim()) ||
          null,
        actorAvatarUrl:
          (typeof latestMsg.author_avatar_url === "string" &&
            latestMsg.author_avatar_url.trim()) ||
          null,
      };
    })
  );
...
export async function buildDerivedFreelancerNotifications(userId) {
  ...
  const [
    ...
    supabase
      ...
      .select("project_id, client_user_id, opened_at")
      .in("project_id", projectIds)
    ...
    supabase
      ...
      .select(
        "id, project_id, event_type, actor_id, actor_display_name, actor_avatar_url, payload, created_at"
      )
      .in("project_id", projectIds)
  ...
  const activityEvents = await overlayLiveUploaderIdentities(
    (activityRes.data ?? []).map((row) => ({
      ...row,
      created_by: row.actor_id,
      created_by_display_name: row.actor_display_name,
      created_by_avatar_url: row.actor_avatar_url,
    })),
    supabase
  ).then((rows) =>
    rows.map((row) => ({
      ...row,
      actor_display_name: row.created_by_display_name,
      actor_avatar_url: row.created_by_avatar_url,
    }))
  );

  /** @type {Record<string, { name: string | null; avatar: string | null }>} */
  const revisionActorByFileId = {};
  for (const ev of activityEvents) {
    if (ev.event_type !== PROJECT_ACTIVITY_EVENT_TYPES.LIBRARY_FILE_APPROVAL_CHANGED) continue;
    const pl = payloadObj(ev.payload);
    if (String(pl.approval_status || "") !== "revision_requested") continue;
    const fileId = pl.file_id ? String(pl.file_id) : "";
    if (!fileId || revisionActorByFileId[fileId]) continue;
    revisionActorByFileId[fileId] = {
      name:
        (typeof ev.actor_display_name === "string" && ev.actor_display_name.trim()) || null,
      avatar:
        (typeof ev.actor_avatar_url === "string" && ev.actor_avatar_url.trim()) || null,
    };
  }
  ...
  const openClientIds = [
    ...new Set(
      (opensRes.data ?? [])
        .map((o) => o.client_user_id)
        .filter(Boolean)
        .map(String)
    ),
  ];
  /** @type {Record<string, { full_name?: string | null; avatar_url?: string | null }>} */
  let openClientProfileMap = {};
  if (openClientIds.length > 0) {
    const { data: openProfiles } = await supabase
      .from("profiles")
      .select("id, full_name, avatar_url")
      .in("id", openClientIds);
    openClientProfileMap = Object.fromEntries(
      (openProfiles ?? []).map((p) => [String(p.id), p])
    );
  }
      ...
      enrichNotification(
        {
          ...
          actorName:
            ...
            clientMap[p.client_id]?.full_name ||
            "Client",
          actorAvatar: u.actorAvatarUrl || projectVisuals(p, clientMap).clientAvatar,
          actions: [
  ...
  for (const f of filesRes.data ?? []) {
    ...
    const revisionActor = revisionActorByFileId[String(f.id)];
    const actorName =
      revisionActor?.name || projectVisuals(p, clientMap).clientName || "A client";
    const actorAvatar =
      revisionActor?.avatar || projectVisuals(p, clientMap).clientAvatar;
    candidates.push(
      enrichNotification(
        {
          type: "file_revision",
          ...
          body: `${actorName} asked for changes on ${fileName}`,
  ...
  for (const open of opensRes.data ?? []) {
    ...
    if (!p) continue;
    const openProf =
      open.client_user_id != null
        ? openClientProfileMap[String(open.client_user_id)]
        : null;
    const openName =
      (typeof openProf?.full_name === "string" && openProf.full_name.trim()) ||
      projectVisuals(p, clientMap).clientName;
    const openAvatar =
      (typeof openProf?.avatar_url === "string" && openProf.avatar_url.trim()) ||
      projectVisuals(p, clientMap).clientAvatar;
    candidates.push(
      enrichNotification(
        {
          ...
          body: `${openName} visited ${p.name || "the project"} for the first time`,
          href: `/project/${p.id}/dashboard`,
          sourceId: String(open.opened_at),
          createdAt: open.opened_at,
          actorName: openName,
          actorAvatar: openAvatar,
          actions: [
