...
/** @param {string} projectId */
function activityActionsHref(projectId) {
  const pid = typeof projectId === "string" ? projectId.trim() : "";
  if (!pid) return null;
  return `/project/${pid}/actions`;
}
  ...
  switch (eventType) {
    ...
    case PROJECT_ACTIVITY_EVENT_TYPES.LIBRARY_FILE_COMMENT: {
      ...
      return {
        ...
        destinationHref: activityLibraryFilesHref(pid, fileId),
        destinationLabel: fileId ? "Open Discussion" : "Open library",
      };
    }
    ...
    case PROJECT_ACTIVITY_EVENT_TYPES.ACTION_CREATED: {
      const title =
      ...
      return {
        ...
        fileLink: title,
        destinationHref: activityActionsHref(pid),
        destinationLabel: "Open actions",
      };
    }
    case PROJECT_ACTIVITY_EVENT_TYPES.ACTION_COMPLETED: {
      const title =
        typeof payload.title === "string" && payload.title.trim()
          ? payload.title.trim()
          : "Action";
