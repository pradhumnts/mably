 ...
 * and to **signed-out marketing visitors** (public `/project/demo-mably` portal).
 *
...
/** Stable author id for the demo client in chat fixtures. */
export const DEMO_CLIENT_USER_ID = "demo-client-user";

/** Default entry for the public (no-auth) demo portal. */
export const DEMO_PORTAL_HREF = `/project/${DEMO_PROJECT_ID}/dashboard`;

/** sessionStorage key for the demo Freelancer ↔ Client view switch. */
export const DEMO_PORTAL_VIEW_STORAGE_KEY = "mably:demo-portal-view";
...
/**
 * True for `/project/demo-mably` and nested portal routes (querystring ignored).
 * Used by middleware to allow unauthenticated marketing visits.
 * @param {unknown} pathname
 * @returns {boolean}
 */
export function isPublicDemoPortalPath(pathname) {
  if (typeof pathname !== "string") return false;
  const path = pathname.split("?")[0]?.split("#")[0] || "";
  return path === `/project/${DEMO_PROJECT_ID}` || path.startsWith(`/project/${DEMO_PROJECT_ID}/`);
}
...
/** Public demo freelancer persona (used when visitors are signed out). */
const DEMO_FREELANCER = Object.freeze({
  name: "Alex Rivera",
  avatar: "/images/alex-rivera.webp",
  email: null,
  userId: null,
});
...
function normalizeFreelancer(freelancer) {
  const name =
    typeof freelancer?.name === "string" && freelancer.name.trim()
      ...
      : DEMO_FREELANCER.name;
  const avatar =
    typeof freelancer?.avatar === "string" && freelancer.avatar.trim()
      ? freelancer.avatar
      : null;
  const email =
...
export async function resolveDemoFreelancerFromSupabase(supabase, user) {
  if (!user) {
    // Public demo visitors — fixed persona with portrait.
    return { ...DEMO_FREELANCER };
  }
...
export function getDemoProjectPortalBundle(freelancer) {
  ...
  return {
    ...
    dashboard: {
      ...
      // Sample booking link so the client demo view can show “Book a call”.
      calendarLink: "https://cal.com/mably-demo",
    },
    meta: {
      isFreelancer: false,
      isDemo: true,
      marketingEmails: false,
      marketingEmailConsentRecorded: false,
    },
  };
}
...
export function getDemoProjectActivityPageData(freelancer) {
  ...
  const activities = [
    ...
    {
      id: "demo-act-action-created",
      eventType: "action.created",
      category: "general",
      user: freelancerUser,
      action: "Added an action:",
      timestamp: "Yesterday",
      fileLink: "Confirm brand color palette",
      destinationHref: `/project/${DEMO_PROJECT_ID}/actions`,
      destinationLabel: "Open actions",
      externalHref: null,
      externalLabel: null,
    },
    {
      ...
      destinationLabel: "Open Discussion",
      externalHref: null,
      externalLabel: null,
    },
    {
      id: "demo-act-action-done",
      eventType: "action.completed",
      category: "general",
      user: clientUser,
      action: "Completed:",
      timestamp: "3 days ago",
      fileLink: "Share competitor references",
      badge: { type: "Approved", icon: "✓", text: "Done" },
      destinationHref: `/project/${DEMO_PROJECT_ID}/actions`,
      destinationLabel: "Open actions",
      externalHref: null,
      externalLabel: null,
...
/**
 * Same shape as `listProjectActions` mapped rows.
 */
export function getDemoProjectActions() {
  return [
    {
      id: "demo-action-1",
      projectId: DEMO_PROJECT_ID,
      title: "Send revised typography PDF",
      notes:
        "<p>Incorporate Maya’s spacing feedback and re-share for approval before Friday’s review.</p>",
      owner: "freelancer",
      visibility: "private",
      dueDate: dateOnly(-1),
      status: "open",
      completedAt: null,
      createdAt: daysAgoIso(3, 9, 15),
    },
    {
      id: "demo-action-2",
      projectId: DEMO_PROJECT_ID,
      title: "Confirm brand color palette",
      notes:
        "<p>Pick a primary + two accents from the options in the style exploration. Reply here or mark done when locked.</p>",
      owner: "client",
      visibility: "shared",
      dueDate: dateOnly(2),
      status: "open",
      completedAt: null,
      createdAt: daysAgoIso(4, 11, 0),
    },
    {
      id: "demo-action-3",
      projectId: DEMO_PROJECT_ID,
      title: "Prep motion principles draft",
      notes:
        "<p>Outline easing, duration tokens, and one sample transition for the hero.</p>",
      owner: "freelancer",
      visibility: "private",
      dueDate: dateOnly(5),
      status: "open",
      completedAt: null,
      createdAt: daysAgoIso(2, 15, 30),
    },
    {
      id: "demo-action-4",
      projectId: DEMO_PROJECT_ID,
      title: "Share competitor references",
      notes:
        "<p>Drop 3–5 brands you admire for tone and typography.</p>",
      owner: "client",
      visibility: "shared",
      dueDate: dateOnly(-4),
      status: "done",
      completedAt: daysAgoIso(3, 16, 40),
...
/**
 * Portal home briefing for the demo project.
 * Covers every attention type the real dashboard can show + a latest activity row.
 *
 * @param {{ name?: string; avatar?: string | null } | null | undefined} freelancer
 * @param {{ isFreelancer?: boolean }} [options]
 */
export function getDemoPortalHomeBriefing(freelancer, options = {}) {
  const fl = normalizeFreelancer(freelancer);
  ...
  const attention = isFreelancer
    ? [
        {
          id: "mine-demo-action-1",
          type: "action",
          title: "Send revised typography PDF",
          subtitle: `Overdue · ${formatDemoShortDate(dateOnly(-1))}`,
          href: `/project/${pid}/actions`,
        },
        {
          id: "waiting-demo-action-2",
          type: "action",
          title: "Confirm brand color palette",
          subtitle: `Waiting on client · Due ${formatDemoShortDate(dateOnly(2))}`,
          href: `/project/${pid}/actions`,
        },
        {
          id: "mine-demo-action-3",
          type: "action",
          title: "Prep motion principles draft",
          subtitle: `Due ${formatDemoShortDate(dateOnly(5))}`,
          href: `/project/${pid}/actions`,
        },
        {
          id: "rev-demo-file-5",
          type: "approval",
          title: "Revision requested · Typography.pdf",
          subtitle: "Client asked for changes",
          href: `/project/${pid}/library/files?discussion=demo-file-5`,
        },
        {
          id: "pend-demo-file-3",
          type: "approval",
          title: "Hero animation.mp4",
          subtitle: "Awaiting client approval",
          href: `/project/${pid}/library/files?discussion=demo-file-3`,
        },
        {
          id: "invoice-demo-invoice-3",
          type: "invoice",
          title: "Unpaid invoice · $2,000",
          subtitle: `Due ${formatDemoShortDate(dateOnly(12))}`,
          href: `/project/${pid}/payments`,
        },
      ]
    : [
        {
          id: "action-demo-action-2",
          type: "action",
          title: "Confirm brand color palette",
          subtitle: `Due ${formatDemoShortDate(dateOnly(2))}`,
          href: `/project/${pid}/actions`,
        },
        {
          id: "file-demo-file-3",
          type: "approval",
          title: "Review Hero animation.mp4",
 ...
 */
function formatDemoShortDate(isoDate) {
...
export function getDemoProjectChatBootstrap(freelancer) {
  ...
  const clientAuthorId = DEMO_CLIENT_USER_ID;
