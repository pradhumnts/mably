...
export async function updateProjectInvoiceStatus(projectId, invoiceId, status) {
  ...
  if (projectRow?.freelancer_id) {
    ...
    if (status === "paid") {
      const { enqueueFreelancerInboxPaymentReceived } = await import(
        "@/lib/notifications/trigger-freelancer-inbox"
      );
      const { data: projName } = await supabase
        .from("projects")
        .select("name")
        .eq("id", pid)
        .maybeSingle();
      enqueueFreelancerInboxPaymentReceived({
