...
export async function prepareLibraryVoiceNoteUpload(payload) {
  ...
  const objectPath = `${projectId}/${VOICE_NOTES_STORAGE_PREFIX}/${crypto.randomUUID()}.${ext}`;
...
const COMMENT_SELECT_VOICE =
  "id, body, author_id, author_display_name, author_avatar_url, created_at, voice_note_storage_path, voice_note_duration_ms, voice_note_mime_type, voice_note_size_bytes, voice_note_waveform, voice_note_transcript";
...
async function insertLibraryFileCommentAndNotify(supabase, user, p) {
  ...
  const activityBody = bodyInsert || (hasVoice ? "[Voice note]" : "");
  await recordProjectActivityEvent(supabase, {
    ...
    payload: {
      ...
      body: activityBody,
      ...(hasVoice ? { voice_note_duration_ms: inserted.voice_note_duration_ms } : {}),
    },
  });
  ...
  if (p.freelancerId) {
    ...
    void notifyPortalLibraryFileComment({
      ...
      preview: bodyInsert || (isVoice ? `Voice note (${durationLabel})` : ""),
      isVoice,
      durationLabel,
  ...
  return { ok: true, comment: { ...inserted, voice_note_listened: false } };
}
 ...
 * } | null} [voicePayload]
 */
export async function addLibraryFileComment(projectId, fileId, body, voicePayload = null) {
  const supabase = await createClient();
  const {
    data: { user },
    error: authErr,
  ...
  const voice = voicePayload && typeof voicePayload === "object" ? voicePayload : null;

  if (!pid || !fid) {
    return { ok: false, error: "Missing id" };
  }
  if (!text && !voice?.storagePath) {
    return { ok: false, error: "Write a message or record a voice note." };
  }
