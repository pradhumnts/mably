...
export const EARLY_OFFER_COPY = {
  ...
  foundingNote:
    "You're in early — one of the first 50 on Mably. Lock in 75% off now; when this cohort fills, founding pricing won't come back.",
  planBarLabel: "LOCK IN BEFORE IT'S GONE",
  planPickerIntro:
    "Pick your plan and claim it today — early pricing on Starter and Growth, only while founding spots last.",
  planSummaryLabel: "50 spots only · 75% off locked in",
...
/**
 * Early-offer dialog layout.
 * - `"option-1"` — portrait / mobile-first (dark)
 * - `"option-2"` — landscape / desktop-friendly (dark) — default
 * - `"option-3"` — landscape / desktop-friendly (light, Mably brand colors)
 *
 * Say "option 1" or "option 2" in chat to switch layouts.
 *
 * @type {"option-1" | "option-2" | "option-3"}
 */
export const EARLY_OFFER_LAYOUT_VARIANT = "option-2";

/** Theme used by the active layout variant (popup + sticky CTA). */
export function getEarlyOfferLayoutTheme() {
  return EARLY_OFFER_LAYOUT_VARIANT === "option-3" ? "light" : "dark";
}
...
export const EARLY_OFFER_PLANS = [
  ...
  },
  {
    ...
    listPriceMonthly: 19,
    features: ["Everything in Starter", "50 GB storage", "Priority support"],
    recommended: true,
  },
];
