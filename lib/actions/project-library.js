...
import {
  overlayLiveUploaderIdentities,
  resolveAuthenticatedActorIdentity,
} from "@/lib/auth/resolve-actor-identity";
import {
  isDemoProjectId,
  getDemoLibraryFiles,
  getDemoLibraryLinks,
...
export async function completeLibraryFileUpload(payload) {
  ...
  const { displayName: createdByDisplayName, avatarUrl: createdByAvatarUrl } =
    await resolveAuthenticatedActorIdentity(supabase, user);
  ...
  await recordProjectActivityEvent(supabase, {
    ...
    actorAvatarUrl: createdByAvatarUrl,
    payload: {
      display_name: displayName,
      needs_approval: needsApproval,
...
export async function listLibraryFiles(projectId) {
  ...
  const fileSelectWithOrigin =
    "id, display_name, original_filename, mime_type, size_bytes, description, needs_approval, approval_status, upload_origin, origin_discussion_file_id, created_by, created_by_display_name, created_by_avatar_url, created_at, version_count, current_version_number, updated_at";
  const fileSelectLegacy =
    "id, display_name, original_filename, mime_type, size_bytes, description, needs_approval, approval_status, created_by, created_by_display_name, created_by_avatar_url, created_at, version_count, current_version_number, updated_at";
  ...
  rows = await overlayLiveUploaderIdentities(rows, supabase);
...
export async function listLibraryLinks(projectId) {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, title, url, description, needs_approval, created_by, created_by_display_name, created_by_avatar_url, created_at"
    )
    .eq("project_id", pid)
    .order("created_at", { ascending: false });

  if (error) return { ok: false, error: error.message, items: [] };
  const items = await overlayLiveUploaderIdentities(data ?? [], supabase);
  return { ok: true, items };
}
...
export async function addLibraryFileComment(projectId, fileId, body, extras = null) {
  ...
  const { displayName: authorDisplayName, avatarUrl: authorAvatarUrl } =
    await resolveAuthenticatedActorIdentity(supabase, user);
