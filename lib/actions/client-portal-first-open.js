...
import { enqueueFreelancerInboxClientOpenedPortal } from "@/lib/notifications/trigger-freelancer-inbox";
...
export async function recordClientPortalFirstOpen(projectId) {
  try {
    ...
    if (!isFirst) return { ok: true, isFirst: false };
    ...
    if (proj?.freelancer_id) {
      enqueueFreelancerInboxClientOpenedPortal({
