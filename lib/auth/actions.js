...
function friendlyAuthError(message, fallback) {
  ...
  if (m.includes("expired")) return "That code expired. Request a new code and try again.";
  if (m.includes("invalid token") || m.includes("token")) {
    return "That code is invalid. Check the latest code in your email and try again.";
  }
  if (m.includes("network")) return "Network issue detected. Check your connection and try again.";
  return fallback;
}
...
export async function signInWithEmail(email) {
  ...
  if (error) {
    ...
    return { error: friendlyAuthError(error.message, "We could not send a sign-in code. Please try again.") }
  }
...
export async function signInWithOAuth(provider, nextPath) {
  ...
  if (error) {
    ...
    return { error: friendlyAuthError(error.message, "Could not start Google sign-in. Please try again.") }
  }
...
export async function verifyOtp(email, token, nextPath) {
  ...
  if (error) {
    ...
    return { error: friendlyAuthError(error.message, "We could not verify that code. Please try again.") }
  }
...
export async function signOut() {
  ...
  if (error) {
    ...
    return { error: "Could not sign you out right now. Please try again." }
  }
