...
function normalizeCommentExtras(extras) {
  ...
  if (typeof o.attachedFileId === "string" && o.attachedFileId.trim()) {
    ids.add(o.attachedFileId.trim());
  }
  return { voice, attachedFileIds: [...ids] };
}
...
async function mapCommentsWithAttachedFiles(supabase, projectId, rows) {
  ...
  if (!missingJunction && junctionRows?.length) {
    for (const link of junctionRows) {
      ...
      const list = fileIdsByComment.get(cid) ?? [];
      list.push(String(link.file_id));
      fileIdsByComment.set(cid, list);
    }
  }

  for (const row of rows) {
    if (fileIdsByComment.has(String(row.id))) continue;
    if (row.attached_file_id) {
      fileIdsByComment.set(String(row.id), [String(row.attached_file_id)]);
    }
  }

  const allFileIds = [...new Set([...fileIdsByComment.values()].flat())];
  ...
  const byId = new Map((files ?? []).map((f) => [String(f.id), mapAttachedLibraryFile(f)]));
  return rows.map((r) => {
    const attached_files = (fileIdsByComment.get(String(r.id)) ?? [])
...
export async function listLibraryFiles(projectId) {
  ...
  const fileSelectWithOrigin =
    "id, display_name, original_filename, mime_type, size_bytes, description, needs_approval, approval_status, upload_origin, origin_discussion_file_id, created_by_display_name, created_by_avatar_url, created_at";
