...
import { enqueueFreelancerInboxProjectCreated } from "@/lib/notifications/trigger-freelancer-inbox";
...
function friendlyProjectActionError(rawMessage, fallback) {
  const m = String(rawMessage || "").toLowerCase();
  if (!m) return fallback;
  if (m.includes("not signed in") || m.includes("jwt") || m.includes("unauthorized")) {
    return "Your session expired. Sign in again and retry.";
  }
  if (m.includes("duplicate key") || m.includes("already exists")) {
    return "This item already exists. Refresh the page and try again.";
  }
...
export async function createProject(form) {
  ...
  const { data: clientRow, error: clientErr } = await supabase
    .from("clients")
    .select("full_name, email, avatar_url, is_sample")
    .eq("id", clientId)
  ...
  if (clientErr || !clientRow) {
    if (clientErr) {
      console.error("[projects] load client:", clientErr.message);
    }
    return {
      ok: false,
      error: friendlyProjectActionError(clientErr?.message, "We could not load that client. Please reselect and try again."),
    };
  }

  if (clientRow.is_sample) {
    return {
      ok: false,
      error: "Sample clients cannot be used for projects. Add a real client or create a new one.",
    };
  }
  ...
  const isFirstProject = (existingProjectCount ?? 0) === 0;
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      total_fee: totalFee,
      milestones,
      brand_color: normalizePortalBrandColorForStorage(form.brandColor),
  ...
  if (error) {
    console.error("[projects] create insert:", error.message);
    return {
      ok: false,
      error: friendlyProjectActionError(error.message, "We could not create your project. Please try again."),
    };
  }
  ...
  if (memberErr) {
    console.error("[projects] create owner membership:", memberErr.message);
    return {
      ok: false,
      error: friendlyProjectActionError(memberErr.message, "Project was created, but we could not finish setup. Please retry."),
    };
  }
  ...
  enqueueFreelancerInboxProjectCreated({
  ...
  if (inviteTo) {
    try {
      const inv = await sendPortalInviteEmail({ projectId, toEmail: inviteTo });
      inviteSent = Boolean(inv?.ok);
    } catch (e) {
      console.error("sendPortalInviteEmail:", e);
    }
  }

  return { ok: true, id: projectId, isFirstProject, inviteSent };
}
...
export async function updateProjectInviteAndResend({
  ...
  if (error) {
    console.error("[projects] update invite:", error.message);
    return {
      ok: false,
      error: friendlyProjectActionError(error.message, "We could not save invite details. Please try again."),
    };
  }
  ...
  if (!inv.ok) {
    return {
      ok: false,
      error: friendlyProjectActionError(inv.error, "Invite details were saved, but we could not send the email. Please retry."),
      id: pid,
    };
  }
...
export async function updatePortalProjectSettings(projectId, fields) {
  ...
  if (fields.brandColor !== undefined) {
    ...
    if (!raw) {
      ...
      brand_color = normalizePortalBrandColorForStorage(normalized);
    }
  }
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      status,
      ...(fields.brandColor !== undefined ? { brand_color } : {}),
    })
    .eq("id", pid)
    .eq("freelancer_id", user.id);
