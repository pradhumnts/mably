...
export async function postLibraryFilesComment({
  ...
  for (let index = 0; index < files.length; index++) {
    ...
    const completed =
      totalFiles === 1
        ? await completeLibraryFileNewVersion({
            projectId: String(projectId),
            ...
            versionNote: typeof body === "string" ? body.trim() : null,
          })
        : await completeLibraryFileUpload({
            projectId: String(projectId),
            objectPath: prep.objectPath,
            displayName,
            originalFilename: prep.normalizedOriginalFilename || file.name,
  ...
  const post = await addLibraryFileComment(
    ...
    typeof body === "string" ? body.trim() : "",
    {
      ...
      attachedVersionIds: succeeded.map((s) => (s.versionId ? String(s.versionId) : null)),
    }
  );
